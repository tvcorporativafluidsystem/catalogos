        ...
        <div className="text-center mb-12 md:mb-20 z-10 mt-16 md:mt-0 leading-none">
          ...
          {isAdmin && (
            <button 
              ...
              className="mt-10 px-8 py-4 bg-green-500 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-green-600 transition-all shadow-2xl animate-bounce leading-none"
            >
...
function CatalogoPage({ marcaInicial, onBack, isAdmin, onLogout }: any) {
  ...
  const temas = {
    URBA: { sidebarBg: 'bg-slate-950', accentColor: '#00A8CC', buttonBg: 'bg-[#00A8CC]', logoUrl: `${STORAGE_URL}/logos/urba_logo.png`, badge: 'bg-[#00A8CC] text-white' },
    BROSOL: { sidebarBg: 'bg-[#2B3990]', accentColor: '#FFD700', buttonBg: 'bg-[#FFD700]', logoUrl: `${STORAGE_URL}/logos/brosol_logo.png`, badge: 'bg-[#FFD700] text-[#2B3990]' }
  };

  const temaAtivo = temas[marca];

  // PESQUISA GERAL LOCAL EM TODOS OS DADOS
    ...
    <div className="flex flex-col lg:flex-row min-h-screen bg-slate-100 font-sans antialiased text-slate-900 leading-none">
      ...
      <header className={`lg:hidden sticky top-0 z-[60] flex items-center justify-between p-4 shadow-xl ${temaAtivo.sidebarBg}`}>
        <div className="h-10 w-32 bg-white rounded-xl overflow-hidden flex items-center justify-center p-1 leading-none">
          <img src={temaAtivo.logoUrl} className="max-h-full max-w-full object-contain bg-white" alt="Logo" />
        </div>
      ...
      <aside className={`fixed lg:sticky top-0 z-[80] w-[300px] sm:w-80 shadow-2xl h-screen p-6 flex flex-col transition-all duration-500 ${temaAtivo.sidebarBg} text-white ${menuAberto ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="mb-10 w-full hidden lg:block"> 
          <div className="bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-white/10 w-full aspect-[2/1] flex items-center justify-center p-4">
             <img src={temaAtivo.logoUrl} className="w-full h-full object-contain bg-white" alt="Logo" />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pr-2 custom-scrollbar pb-10">
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 bg-black/30 p-1 rounded-xl border border-white/10 leading-none font-sans">
          ...
          <div className="space-y-6 text-white leading-none font-sans">
            ...
            <div className="relative">
              <div className="flex justify-between items-center mb-2">
                <label className="text-[10px] font-black text-white/40 uppercase ml-1">Código Urba/Brosol</label>
            ...
            <div className="relative">
              <div className="flex justify-between items-center mb-2">
                <label className="text-[10px] font-black text-white/40 uppercase ml-1 font-sans">Linha (Grupo)</label>
          ...
          <button onClick={() => setMenuAberto(false)} className="lg:hidden w-full py-4 rounded-2xl font-black text-xs uppercase shadow-xl bg-white text-slate-900 leading-none">Aplicar Filtros</button>
        </div>
      </aside>

      <main className="flex-1 p-4 lg:p-10 overflow-y-auto leading-none">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 text-slate-900 leading-none">
              {produtosExibidos.map(p => (
                <div key={p.id} className="bg-white rounded-[2.5rem] shadow-sm hover:shadow-2xl transition-all border border-slate-100 flex flex-col group overflow-hidden leading-none">
                  ...
                  <div className="p-8 flex flex-col flex-1 leading-none text-slate-900">
                    ...
                    <p className="text-xs text-slate-400 line-clamp-3 mb-6 italic leading-tight font-sans leading-none">{p.dados['Veículos']}</p>
...
function ModalDetalhes({ produto, marca, storageUrl, onClose, temaAtivo }: any) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 sm:p-4 bg-slate-900/95 backdrop-blur-md leading-none font-sans">
      <div className="relative bg-white w-full h-full lg:h-auto lg:max-h-[95vh] lg:max-w-6xl lg:rounded-[3rem] shadow-2xl overflow-hidden flex flex-col lg:flex-row border border-slate-100 text-slate-900 leading-none">
        ...
        <div className="lg:w-1/2 bg-slate-50 p-8 flex flex-col items-center justify-center min-h-[400px] relative text-slate-900 leading-none">
          ...
          {fotos.length > 1 && (
            <div className="flex gap-3 mt-6 p-2 overflow-x-auto max-w-full custom-scrollbar text-slate-900 leading-none">
              {fotos.map((url, i) => (
